...
@Builder
public class BuildingTextureSystem implements System, ComponentLifecycleListener<BuildingState> {
    ...
    public BuildingTextureSystem(
        ...
        this.farm = farm;
        this.field = field;
        Entities.allComponents(BuildingState.class).forEach(this::onComponentAttached);
    }
    ...
    public void update(final Duration delta) {
        ...
        Entities.allComponents(BuildingState.class).forEach(buildingStateComponent ->
    ...
    @Override
    public void onComponentAttached(final Component<BuildingState> component) {
        component.getEntity().attachComponent(getBuildingTexture(component.getState()));
    }

    public TransparentTexture getBuildingTexture(final BuildingState buildingState) {
        final BuildingType buildingType = buildingState.getBuildingType();
        switch (buildingType) {
            case FARM:
                return farm;
            case FIELD:
                return buildingState.isBuilt() ? field : dirt;
            default:
